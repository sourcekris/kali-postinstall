...
xfce_monitor() {
    local monitor=$(xfconf-query -c xfce4-desktop -l | awk -F'/' '{print $3"/"$4}' | grep screen | grep -v -E "monitor[0-4]" | sort | uniq)
    echo $monitor
}

# Change desktop background settings
xfce_desktop_bg() {
    local monitor=$(xfce_monitor)
    local bg="/backdrop/$monitor/workspace0"
...
# xfce_check_panel_exists returns 1 if the panel $panel_no already exists.
xfce_check_panel_exists() {
    local panel_no="$1"
    xfconf-query -c xfce4-panel -p /panels | grep -qE "^$panel_no$"
    if [ $? -eq 0 ];
    then
...
# Create top panel (panel-2)
xfce_create_top_panel() {
    local panel_height=34
    local panel_width=100

    xfce_check_panel_exists 2
    if [ $? -eq 1 ];
    then
        log "warning" "panel 2 already exists in xfce4 conf, skipping creation"
        return 1
    fi
    ...
    xfconf-query -c xfce4-panel -pn /panels/panel-2/length -t int -s $panel_width
    xfconf-query -c xfce4-panel -pn /panels/panel-2/size -t int -s $panel_height
}

xfce_remove_default_plugins() {
